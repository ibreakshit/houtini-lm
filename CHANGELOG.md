...
### Added
- **Per-call usage telemetry** (`call_log`) — local-model calls now record bounded fire-and-forget usage rows in the existing SQLite cache, including model, tier, tool, token counts, latency, throughput, and success status. The `stats` tool appends a usage breakdown by tier and tool without changing existing behavior. Configure or disable with `HOUTINI_LM_TELEMETRY`, `HOUTINI_LM_TELEMETRY_MAX`, and `HOUTINI_LM_HOME`.
